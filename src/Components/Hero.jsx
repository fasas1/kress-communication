...
import videoFile from "../../public/assets/showreel.mp4";
  ...
  return (
    ...
    >
      ...
      <div className="relative z-30 text-center text-white px-4 animate-slideFadeUp">
        ...
        <p className="text-sm mt-2 text-gray-400">Capturing stories, creating magic.</p>
      </div>

      {/* 🎬 Gradient Glassy Play Button */}
      <button
        onClick={() => setIsOpen(true)}
        className="absolute z-40 w-13 h-13 rounded-full bg-gradient-to-r from-pink-500 via-purple-500 to-blue-500 flex items-center justify-center text-white text-4xl shadow-xl border border-white/20 hover:scale-110 transition-all duration-300 animate-pulseGlow group"
      ...
      {/* 🎥 Video Modal */}
      {isOpen && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 animate-fadeInSlow">
          <div className="relative w-[90%] max-w-3xl aspect-video bg-black rounded-lg overflow-hidden shadow-2xl">
            <video src={videoFile} controls autoPlay className="w-full h-full object-cover" />
            <button
              onClick={() => setIsOpen(false)}
              className="absolute top-3 right-3 bg-black/70 rounded-full px-3 py-1 text-lg hover:bg-red-600"
            >✕</button>
          </div>
        </div>
      )}
      ...
      <style>{`
        ...
        @keyframes slideFadeUp { 0% { opacity: 0; transform: translateY(30px) scale(1.02); } 100% { opacity: 1; transform: translateY(0) scale(1); } }
        @keyframes pulseGlow { 0% { box-shadow: 0 0 15px rgba(236, 72, 153, 0.4), 0 0 30px rgba(147, 51, 234, 0.2); } 50% { box-shadow: 0 0 25px rgba(236, 72, 153, 0.7), 0 0 50px rgba(147, 51, 234, 0.5); } 100% { box-shadow: 0 0 15px rgba(236, 72, 153, 0.4), 0 0 30px rgba(147, 51, 234, 0.2); } }
